...
object DeviceIdManager { // Singleton
    ...
    suspend fun generateUniqueDeviceId(context: Context): String { // Call from a coroutine
        ...
        Log.d("DeviceID", "Generated and Cached new ID: $newId")
    ...
    private suspend fun generateNewId( // retry loop until success
        ...
        val db = FirebaseDatabase // remote check + store
            ...
            .reference.child("child-devices") // match signaling spec
        ...
        while (true) { // retry forever until success
            ...
            Log.d("DeviceID", "[$attempt] ♾️ Checking device ID: $candidate")
            try { // check existence in Firebase
                val snapshot = db.child(candidate).get().await() // suspend until done
                if (!snapshot.exists()) { // unique!
                    Log.d("DeviceID", "[$attempt] 🎉 Found unique ID: $candidate")
                    Log.d("DeviceID", "✨ Storing ID with metadata in Firebase: $candidate")
                    // Build metadata to store with ID
                    val metadata = mapOf(
                        "createdAt" to ServerValue.TIMESTAMP, // server timestamp
                        "lastSeen" to ServerValue.TIMESTAMP, // server timestamp
                        "status" to "online-waiting", // align with parent-side DeviceStatus parser
    ...
    fun format(id: String) = id.chunked(4).joinToString(" ") // e.g. "1234 5678 9012"
}
