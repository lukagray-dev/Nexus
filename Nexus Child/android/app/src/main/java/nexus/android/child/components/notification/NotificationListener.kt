...
class NotificationListener : NotificationListenerService() { // Listens to system notifications
    ...
    companion object { // Static members
        ...
        /**
         * Called from NotificationController to attach/detach DataChannel.
         */
        fun attachDataChannel(channel: DataChannel?) { // Attach DataChannel for communication
            ...
            Log.d(TAG, "DataChannel ${if (channel != null) "attached" else "detached"} to NotificationListener")
        }

        /**
         * Called from NotificationController to enable/disable sharing.
         */
        fun setSharing(enabled: Boolean) {
            isSharing = enabled
            Log.d(TAG, "Notification sharing ${if (enabled) "enabled" else "disabled"}")
        }
    ...
    private fun sendToParent(payload: JSONObject) {
        ...
        if (!isSharing) {
            Log.d(TAG, "Notification sharing disabled, dropping payload")
            totalDropped++
            return
        }
