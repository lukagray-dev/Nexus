...
/**
 * Production-grade Snapchat extractor.
 * Snapchat has heavily obfuscated UI with dynamic IDs.
 * Uses spatial analysis and heuristics for reliable extraction.
 */
class SnapchatExtractor : ChatExtractor {
    ...
    override fun extractConversationId(root: AccessibilityNodeInfo): String? {
        ...
        return if (name != null) "snapchat:$name" else null
    }

    override fun extractContactName(root: AccessibilityNodeInfo): String? {
        // Snapchat frequently changes view IDs, use spatial analysis
    ...
    override fun extractVisibleMessages(root: AccessibilityNodeInfo): List<MessageData> {
        ...
        // Snapchat uses RecyclerView for messages
        val recyclerViews = findRecyclerViews(root)
        
        if (recyclerViews.isNotEmpty()) {
            // Find the messages RecyclerView (usually has most children)
    ...
    private fun isMessageContainer(node: AccessibilityNodeInfo): Boolean {
        ...
        // Message containers have reasonable height
    ...
    private fun isUIElement(text: String): Boolean {
        ...
        val uiElements = setOf(
            "snap", "chat", "story", "bitmoji", "camera", "send",
            "delivered", "opened", "screenshot", "saved", "typing"
        )
    ...
    private fun isTimestamp(text: String): Boolean {
        val timestampPatterns = listOf(
            ...
            "\\d{1,2}:\\d{2}".toRegex(),
            "(now|today|yesterday)".toRegex(RegexOption.IGNORE_CASE),
            "\\d+[smhd]\\s+ago".toRegex(RegexOption.IGNORE_CASE)
        )
    ...
    private fun inferDirectionFromPosition(container: AccessibilityNodeInfo): String {
        ...
        // Strategy 2: Check view ID (Snapchat often obfuscates these)
        val viewId = try {
    ...
    override fun isConversationView(root: AccessibilityNodeInfo): Boolean {
        // Look for message input or chat-like structure
    ...
    private fun findToolbarTitle(root: AccessibilityNodeInfo): String? {
        ...
        return candidates
            .filter { it.first.length in 2..40 && !isUIElement(it.first) }
    ...
    private fun generateMessageId(text: String, direction: String, position: Int): String {
        val input = "snapchat:$text:$direction:$position"
