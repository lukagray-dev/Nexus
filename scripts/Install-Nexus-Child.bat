@echo off
REM ============================================================
REM Nexus Child Android - Installation and Build Script
REM ============================================================
REM This script builds and installs the Nexus Child APK on a 
REM connected Android device or emulator.
REM ============================================================

setlocal enabledelayedexpansion
set "ROOT_DIR=%~dp0.."
set "CHILD_PROJECT=%ROOT_DIR%\Nexus Child\android"

echo.
echo ============================================================
echo Nexus Child Android - Installation Script
echo ============================================================
echo.

REM Verify project directory exists
if not exist "%CHILD_PROJECT%" (
  echo [ERROR] Child Android project not found at:
  echo         "%CHILD_PROJECT%"
  echo.
  exit /b 1
)
...
cd /d "%CHILD_PROJECT%"
if errorlevel 1 (
  echo [ERROR] Failed to navigate to Child Android project directory.
  exit /b 1
)
...
if errorlevel 1 (
  echo [ERROR] Failed to build Nexus Child debug APK.
...
echo        %CHILD_PROJECT%\app\build\outputs\apk\debug\app-debug.apk
echo.
...
if errorlevel 1 (
  ...
  echo        3. Run: adb install -r "%CHILD_PROJECT%\app\build\outputs\apk\debug\app-debug.apk"
  echo.
  exit /b 0
)
...
call adb install -r "%CHILD_PROJECT%\app\build\outputs\apk\debug\app-debug.apk"
if errorlevel 1 (
...
echo.
echo ============================================================
echo [SUCCESS] Nexus Child installed successfully!
echo ============================================================
echo.
echo [INFO] The app should now be available on your device.
echo        Package: nexus.android.child
