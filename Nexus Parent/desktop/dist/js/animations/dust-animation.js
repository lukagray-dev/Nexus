...
const FastSimplexNoise = (function() {
    ...
    Noise.prototype = {
/**
 * Generates a noise value based on the input coordinates.
 *
...
class DustAnimation {
    /**
     * Constructor for the DustAnimation class.
    ...
    /**
     * Sets up event listeners for window resize and canvas mousemove events.
     * This is needed to keep the animation responsive to window resizes and
    ...
    /**
     * Sets up the WebGL context for rendering the animation.
     * This includes compiling the vertex and fragment shaders, linking the program,
     * and setting up the attribute and uniform locations.
     */
    setupWebGL() {
        const vsSource = `
        ...
        /**
         * Resize the canvas to match the actual display size of the canvas in the DOM.
         * This function is called when the window is resized.
         * It gets the actual display size of the canvas using offsetWidth/offsetHeight, and
         * sets the canvas resolution (not just CSS size) to match the display size.
         * It also updates the WebGL viewport to match the new canvas size.
         */
    resize() {
        ...
        /**
         * Initializes the particle system by creating a specified number of particles and
    ...
    animate = () => {
        ...
        for (let i = 0; i < this.particles.length; i++) {
            this.particles[i].update(this.noise, this.time, this.config, this.width, this.height, this.mouseX, this.mouseY);
        }
        ...
        this.gl.uniform3f(this.uColor, this.config.color.r / 255.0, this.config.color.g / 255.0, this.config.color.b / 255.0);
...
class DustParticle {
/**
 * Constructor for a DustParticle.
 ...
 */
    ...
    /**
     * Resets the particle to a random position, size, and alpha value.
     * If initial is true, the particle's life is also reset to a random value.
     * @param {boolean} [initial=false] - Whether to reset the particle's life.
     */
    reset(initial = false) {
    ...
    /**
     * Updates the particle's position based on the Perlin noise value at the current position,
     * time, and configuration.
     * The particle is also affected by gravity, bouncing off the edges of the canvas, and
     * reacting to the user's mouse position.
     * If the particle's life reaches 0, it resets to a random position, size, and alpha value.
     * @param {SimplexNoise} noise - A Perlin noise implementation.
     * @param {number} time - The current animation time.
     * @param {Object} config - The configuration object containing animation settings.
     * @param {number} width - The width of the canvas.
     * @param {number} height - The height of the canvas.
     * @param {number} mouseX - The user's mouse X position.
     * @param {number} mouseY - The user's mouse Y position.
     */
